...
class Visualizador:
    ...
    def _obter_tipo_agente(self, agente_id: str, agentes: List[Agente] = None) -> str:
        """Retorna o tipo do agente como abreviação (Q, G, R)"""
            ...
            for agente in agentes:
                if agente.agente_id == agente_id:
                    ...
                    if 'QLearning' in tipo_classe or 'Q-Learning' in tipo_classe:
                        ...
                        tipo = agente_id[0].upper()
                    self.tipos_agentes[agente_id] = tipo
                    return tipo
        
        # Fallback: usar primeira letra do ID
        return agente_id[0].upper()
    ...
    def atualizar(self, passo: int, agentes: List[Agente] = None):
        ...
        if agentes:
            for agente in agentes:
                tipo_classe = type(agente).__name__
                if 'QLearning' in tipo_classe or 'Q-Learning' in tipo_classe:
                    self.tipos_agentes[agente.agente_id] = "Q"
                elif 'Evolucionario' in tipo_classe or 'Genetico' in tipo_classe:
                    self.tipos_agentes[agente.agente_id] = "G"
                elif 'Reativo' in tipo_classe:
                    self.tipos_agentes[agente.agente_id] = "R"
                else:
                    self.tipos_agentes[agente.agente_id] = agente.agente_id[0].upper()
        # Processar eventos
